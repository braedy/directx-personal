...
//take d3d device and load texture file to shader resource variable
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd){
	bool result;

	// init new shaders
	result = InitializeShader(device, hwnd, L"texture_vert.hlsl", L"texture_pix.hlsl");
	if (!result) return false;
	return true;
}

//release shaders, variables and related objects
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename){
	...
	if (FAILED(result)){
		//if failed to compile, printout error
		if (errorMessage){
	...
	// the input layout has changed, we now have texure element instead of color
	// the first position element stays unchanged but the SemanticName and
	// Format of the second element have been changed to TEXCOORD and
	// DXGI_FORMAT_R32G32_FLOAT. these two changes will now align this
	// layout with the VertexType in both the ModelClass definition and the
	// typedefs in the shader files.
	...
	samplerDesc.Filter = D3D11_FILTER_ANISOTROPIC;//MIN_MAG_MIP_LINEAR;
