...
bool ModelListClass::Initialize(int numModels){
	...
	for (i = 0; i < m_modelCount; i++){
		...
		m_ModelInfoList[i].colour = XMFLOAT4(red, green, blue, 1.0f);
...
void ModelListClass::Shutdown(){
	//release model information list
	if (m_ModelInfoList){
...
void ModelListClass::GetData(int index, float& positionX, float& positionY, float& positionZ, XMFLOAT4& color){
	positionX = m_ModelInfoList[index].posX;
	positionY = m_ModelInfoList[index].posY;
	positionZ = m_ModelInfoList[index].posZ;

	color = m_ModelInfoList[index].colour;
}