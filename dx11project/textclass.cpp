...
bool TextClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd, int screenWidth, int screenHeight,
	...
	result = m_Font->Initialize(device, deviceContext, "fontdata.txt", "font.tga");
	if (!result){
	...
	result = UpdateSentence(m_sentence1, "This sentence", 20, 20, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result) return false;
	...
	result = UpdateSentence(m_sentence2, "Renders correct", 20, 40, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result) return false;
	...
	XMMATRIX &orthoMatrix)
{
	...
	pixelColour = XMFLOAT4(sentence->red, sentence->green, sentence->blue, 1.0f);
...
bool TextClass::SetRotation(int count, ID3D11DeviceContext* deviceContext){
	char tempString[32];
	char countString[32];
	bool result;
	...
	// Setup the render count string.
	strcpy_s(countString, "Rotation Y: ");
	strcat_s(countString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, countString, 20, 20, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result) return false;
