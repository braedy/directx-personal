...
bool DeferredBuffersClass::Initialize(ID3D11Device* device, int textureWidth, int textureHeight, float screenDepth, float screenNear){
	...
	//create render target textures and place in array
	for (i = 0; i < BUFFER_COUNT; i++){
	...
	//create render target views and place in array
	for (i = 0; i < BUFFER_COUNT; i++){
...
//sets all render targets in array as the shader writing location
//(instead of back buffer or single render texture target)
void DeferredBuffersClass::SetRenderTargets(ID3D11DeviceContext* deviceContext){
	//bind render target view array and depth stencil buffer to the output render pipeline
...
//get pointer to shader resource in array
